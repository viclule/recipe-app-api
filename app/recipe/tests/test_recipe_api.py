...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_create_recipe_with_ingredients(self):
        ingredient1 = sample_ingredient(user=self.user, name='Prans')
...
class RecipeImageUploadTests(TestCase):
    ...
    def test_upload_image_to_recipe(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.jpg') as ntf:
            ...
            ntf.seek(0)  # pointer back to the beginning of the file
    ...
    def test_filter_recipes_by_tags(self):
        ...
        recipe1 = sample_recipe(user=self.user, title='Recite for test')
        recipe2 = sample_recipe(user=self.user, title='Recipe 2')
        tag1 = sample_tag(user=self.user, name='Vegan')
        tag2 = sample_tag(user=self.user, name='Vegetarian')
        recipe1.tags.add(tag1)
        recipe2.tags.add(tag2)
        recipe3 = sample_recipe(user=self.user, title='some fish')
    ...
    def test_filter_recipes_by_ingredient(self):
        """Test returning recipes with sprecific ingredients"""
        recipe1 = sample_recipe(user=self.user, title='Recite for test')
        recipe2 = sample_recipe(user=self.user, title='Recipe 2')
